...
def task_red_line(frame):
    ...
    red_left = read_color(S2_LEFT, S3_LEFT, OUT_LEFT)
    red_right = read_color(S2_RIGHT, S3_RIGHT, OUT_RIGHT)
    kirmizi_algilandi_sol = red_left < 200
    kirmizi_algilandi_sag = red_right < 200
        
    if kirmizi_algilandi_sol and not onceki_kirmizi_durum_sol:
        red_count += 1
        onceki_kirmizi_durum_sol = True

        if(not kirmizi_algilandi_sol):
            onceki_kirmizi_durum_sol = False
            
        if(kirmizi_algilandi_sag and not onceki_kirmizi_durum_sag):
            red_count += 1
            onceki_kirmizi_durum_sag = True

        if(not kirmizi_algilandi_sag):
            onceki_kirmizi_durum_sag = False
